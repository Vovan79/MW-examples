...
import { FONTS_COLUMNS as columns } from '../../../../constants/columns.constants';
...
const FontsTable: React.FC<Props> = ({ title }) => {
  ...
  const fontsData = useSelector(selectFontsInfo);
  ...
  return (
    <div className={classes.container}>
      <TableHeader title={title} font={FONT} />
      <TableContainer component={Paper}>
        ...
        >
          ...
          />
          <TableBody>
            {stableSort(fontsData, getComparator(order, orderBy))
              ...
              .map((row: any) => (
                ...
                <TableRow className={classes.row} key={row.id} onDoubleClick={() => handleShowDetails(row.id)}>
                  ...
                  <TableCell
                    ...
                    align="center"
                  >
                    <ActionsBlock id={row.id} font={FONT} short />
                  </TableCell>
                </TableRow>
              ))}
          </TableBody>
